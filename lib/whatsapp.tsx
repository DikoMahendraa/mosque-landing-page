...
export type RegistrationKind = "acara" | "kajian"

export function buildRegistrationUrl(title: string, kind: RegistrationKind) {
  const label = kind === "kajian" ? "Kajian" : "Acara"
  ...
  const message = [
    ...
    "",
    `${label}: ${title}`,
    `Izin mendaftar pada ${kind} "${title}" yang tertera.`,
    "",
    "Terima kasih.",
  ].join("\n")
...
export function buildEventRegistrationUrl(eventTitle: string) {
  return buildRegistrationUrl(eventTitle, "acara")
}
